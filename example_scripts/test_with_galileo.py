...
def run_batch_inference_job(
    ...
    dq.set_split("test")
    dq.set_epoch(0)
    evaluate(refined=refined,
             evaluation_dataset_name_to_docs=evaluation_dataset_name_to_docs,
             el=False,  # only evaluate EL when training EL
...
def main():
    ...
    print("Data dir:", fine_tuning_args.data_dir)
    ...
    for i, batch_start_idx in enumerate(range(0, entity_type_ids.shape[0], BATCH_SIZE)):
        print (f"Starting Batch {i} with entity id range {entity_type_ids[batch_start_idx]} - {entity_type_ids[batch_start_idx + BATCH_SIZE - 1]}")
        ...
        # Create a Galileo Inference Job
